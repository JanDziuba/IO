<?php
    ...
    function handleStartTrainingRequest() {
        // Sprawdzamy czy użytkownik jest zalogowany
        if (!isLogged()) {
            return generateResult(NO_LOGGED_USER);
        }
        // Sprawdzamy czy dostaliśmy poprawnego requesta
        if (!isset($_POST['batchSize']) || !isset($_POST['trainingRepetitions']) || !isset($_POST['lessonId'])) {
        ...
        try {
            $createdTraining = startTraining(getLogin(), $lessonId, intval($batchSize), intval($trainingRepetitions), $name);
    ...
    session_start();
    echo handleStartTrainingRequest();
?>
